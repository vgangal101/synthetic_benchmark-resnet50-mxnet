...
def train(opt, ctx):
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        tic = time.time()
        train_data.reset()
        metric.reset() # if u turn this off u get about 1000 img/sec ++
        img_secs=[]
        btic = time.time()
        for i, batch in enumerate(train_data):
            ...
            metric.update(label, outputs) # if u turn this off u get about 1000 img/sec ++
            
            if opt.log_interval and not (i+1)%opt.log_interval:
                name, acc = metric.get()
                logger.info('Epoch[%d] Batch [%d]\tSpeed: %f samples/sec\t%s=%f, %s=%f'%(epoch, i, batch_size/(time.time()-btic), name[0], acc[0], name[1], acc[1]))
            
        #    print("step %d img/sec=%.4f"%(i,batch_size/(time.time()-btic)))
        #    img_secs.append(batch_size/(time.time()-btic))
            btic = time.time()
        epoch_time = time.time()-tic
        #print("img/sec mean= %.4f"%(np.mean(img_secs)))
